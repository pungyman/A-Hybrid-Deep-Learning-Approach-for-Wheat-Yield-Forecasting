"""
Optuna HPO for the Parallel CNN-LSTM-Attention baseline.

Usage:
    python hpo_parallel.py --config config/parallel_with_soil.yaml
    python hpo_parallel.py --config config/parallel_without_soil.yaml

Search space (from action_items.md §2.3):
    conv1_channels:  16–64,  step 8
    conv2_channels:  32–128, step 16
    lstm_hidden:     32–128, step 16
    lstm_layers:     1–3
    lstm_dropout:    0.0–0.5
    attention_dim:   {16, 32, 64, 128}
    n_fc_layers:     2–4
...
from parallel_cnn_lstm_attn import ParallelCNNLSTMAttn
...
def objective(trial, config):
    ...
    lstm_hidden = trial.suggest_int('lstm_hidden', 32, 128, step=16)
    lstm_layers = trial.suggest_int('lstm_layers', 1, 3)
    lstm_dropout = trial.suggest_float('lstm_dropout', 0.0, 0.5)
    attention_dim = trial.suggest_categorical('attention_dim', [16, 32, 64, 128])
...
def run_hpo(config):
    ...
    study.optimize(
        ...
        timeout=config['optimization'].get('timeout_hours', 6) * 3600,
    )
...
def main():
    parser = argparse.ArgumentParser(description='HPO for Parallel CNN-LSTM-Attn baseline')
    parser.add_argument('--config', default='config/parallel_with_soil.yaml', help='Config file path')
    ...
    config['optimization'].setdefault('study_name', 'parallel_baseline')
    config['optimization'].setdefault('hpo_dir', '../../experiments/hpo/parallel')
    config['optimization'].setdefault('n_trials', 300)
    config['optimization'].setdefault('timeout_hours', 6)
