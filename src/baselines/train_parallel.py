"""
Training driver for the Parallel CNN-LSTM-Attention baseline (Song et al. 2025 TPCLA-inspired).

Usage:
    python train_parallel.py --config config/parallel_with_soil.yaml
    python train_parallel.py --config config/parallel_without_soil.yaml

Follows the shared protocol from paper_revision/action_items.md §2.0 / §2.3:
- L2 (MSE) loss only for training and early stopping
- Adam optimizer (LSTM-based model)
...
def setup_parallel_model(config, input_features, flat_soil_dim, device):
    """Instantiate ParallelCNNLSTMAttn, loss, and optimizer from config."""
    mc = config['model']
    tc = config['training']

    model = ParallelCNNLSTMAttn(
        input_features=input_features,
        conv1_channels=mc['conv1_channels'],
        conv2_channels=mc['conv2_channels'],
...
def main(config_path: str):
    try:
        ...
        for run_idx, seed in enumerate(seeds):
            ...
            model, criterion, optimizer = setup_parallel_model(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train Parallel CNN-LSTM-Attn baseline for yield prediction')
