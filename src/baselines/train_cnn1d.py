"""
Training driver for the 1D-CNN baseline (Wolanin 2020 / Nevavuori 2019 style).

Usage:
    python train_cnn1d.py --config config/cnn1d_with_soil.yaml
    python train_cnn1d.py --config config/cnn1d_without_soil.yaml

Follows the shared protocol from paper_revision/action_items.md §2.0 / §2.1:
...
def setup_cnn1d_model(config, input_features, flat_soil_dim, device):
    """Instantiate CNN1D, loss, and optimizer from config."""
    mc = config['model']
    tc = config['training']
...
def main(config_path: str):
    try:
        ...
        for run_idx, seed in enumerate(seeds):
            ...
            model, criterion, optimizer = setup_cnn1d_model(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train 1D-CNN baseline for yield prediction')
