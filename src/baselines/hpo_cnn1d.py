"""
Optuna HPO for the 1D-CNN baseline.

Usage:
    python hpo_cnn1d.py --config config/cnn1d_with_soil.yaml
    python hpo_cnn1d.py --config config/cnn1d_without_soil.yaml
    python hpo_cnn1d.py --config config/cnn1d_with_soil.yaml --n-trials 50

Search space (from action_items.md §2.1):
...
def main():
    parser = argparse.ArgumentParser(description='HPO for 1D-CNN baseline')
    parser.add_argument('--config', default='config/cnn1d_with_soil.yaml', help='Config file path')
    ...
    # Merge optimization defaults if not present in config
    config.setdefault('optimization', {})
    config['optimization'].setdefault('study_name', 'cnn1d_baseline')
    config['optimization'].setdefault('hpo_dir', '../../experiments/hpo/cnn1d')
